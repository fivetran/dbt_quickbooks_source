...
- Add financial statement ordinals in seed files `quickbooks__financial_statement_ordinal` to allow ordering functionality in `quickbooks__balance_sheet` and `quickbooks__profit_and_loss` in the `dbt_quickbooks` package. The ordering is based off general best practices for ordering `account_class` by default, but can be customized further with `account_type`,`account_sub_type`, `account_number` fields if needed. [#36](https://github.com/fivetran/dbt_quickbooks_source/pull/36)
